...
EncodedRPS = Literal["A", "B", "C", "X", "Y", "Z"]
...
def test_actual_input_returns_13446():
...
def parse_input_line(line: str) -> Optional[Tuple[RPS, RPS]]:
    m = re.match(input_line_re, line)
    if not m or not m.group('abc') or not m.group('xyz'):
        return None
    abc = cast(EncodedRPS, m.group('abc'))
    xyz = cast(EncodedRPS, m.group('xyz'))
    if abc and xyz:
        return (char_to_rps(abc), char_to_rps(xyz))
    else:
        return None
...
def score_from_lines(f: Iterable[str]) -> Score:
    ...
    for line in f:
        ...
        if not parsed:
            continue
        abc, xyz = parsed
        totes = Score(totes + wld_to_score(win_lose_draw(abc, xyz)) +
                      score_for_rps(xyz))
    return totes
